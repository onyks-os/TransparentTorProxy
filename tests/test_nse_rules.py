...
def test_routable_icmpv6_is_not_allowed_to_escape(ns_sandbox, ttp_ruleset) -> None:
    ...
    The sniffer's BPF default excluded *all* of ICMPv6 through nse 2.1.1, so a
    routable echo never reached userspace to be classified. nse 2.1.2 narrowed
    the exclusion to Neighbour Discovery (types 133-137), which is why it is
    the floor; `tests/test_nse_classifier.py` asserts that the classifier calls
    such an echo a leak once it arrives.

    The positive control is what caught this: on 2.1.1 the echo left the
    socket and was never captured, so the test failed rather than asserting
    containment of a packet it could not see. `icmp_to` reports its send, so
    if it fails again the control names which it is: a raw sendto the kernel
    refused, or a packet that left the socket and was lost before the capture.
    """
    ns, loop = ns_sandbox
    if not has_ipv6(ns):
        pytest.skip("no IPv6 default route in this sandbox")
