...
@patch("ttp.tor_install.pwd.getpwnam")
@patch("ttp.tor_install.os.chown")
@patch("ttp.tor_install.os.chmod")
def test_generate_torrc_creates_file(mock_chmod, mock_chown, mock_getpwnam, tmp_path: Path):
    """generate_torrc writes a valid torrc to the runtime directory."""
    runtime_dir = tmp_path / "run/tor"
    cache_dir = tmp_path / "lib/cache"

    mock_getpwnam.return_value = MagicMock(pw_uid=111, pw_gid=111)

    with (
        patch.object(tor_install, "TOR_RUNTIME_DIR", runtime_dir),
        patch.object(tor_install, "TOR_CACHE_DIR", cache_dir),
    ):
        torrc_path = generate_torrc("debian-tor")
        ...
        assert "ControlSocket /run/tor/ttp/control.sock" in content
        assert f"DataDirectory {cache_dir}" in content
        assert "User debian-tor" in content

        # Verify chown was attempted for cache and runtime dirs
        assert mock_chown.call_count == 2
        mock_chown.assert_any_call(cache_dir, 111, 111)
        mock_chown.assert_any_call(runtime_dir, 111, 111)
