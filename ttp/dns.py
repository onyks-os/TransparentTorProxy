...
def apply_dns(interface: str, disable_ipv6: bool = False, dns_port: int = 9054) -> dict[str, Any]:
    ...
    try:
        ...
        #    it, root truncates and overwrites whatever the link points at. The
        #    same descriptor is then handed to mount(8) as /proc/self/fd/<n>, so
        #    the inode that gets published at /etc/resolv.conf is provably the
        #    one just written and cannot be swapped in between.
        ...
        try:
            ...
            # 4. Overlay via mount --bind (non-destructive), pinned to the fd.
            subprocess.run(
                [resolve("mount"), "--bind", f"/proc/self/fd/{fd}", str(target)],
                capture_output=True,
                text=True,
                check=True,
                timeout=10,
